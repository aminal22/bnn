...
from torch.autograd import Variable  # No need for this anymore
...
def test(evaluate=False):
    ...
    print(f'\nTest set: Average loss: {test_loss * args.batch_size:.4f}, Accuracy: {correct}/{len(test_loader.dataset)}'
          f' ({100. * float(correct) / len(test_loader.dataset):.2f}%)')
